...
void* web(void * data)
{
    ...
    /* send file in 8KB block - last block may be smaller */ 
            char* hashText = (char*)malloc(BUFSIZ * 4);

    FILE* fp;
    pthread_mutex_lock(&cachewrite);
    fp = fopen("/home/liux7/Desktop/Linux/web/log/hash.txt", "w+");
    
    if(get_str_by_str(table, buffer, &hashText) == HASHOK)
    {
        // pthread_mutex_lock(&cachewrite);
        (void) write(fd, hashText, strlen(hashText));
        fprintf(fp,"in hash content:%s size:%ld \n", hashText, strlen(hashText));
        // pthread_mutex_unlock(&cachewrite);
    }
    else
    {
        fprintf(fp,"hash fail\n");

        while ((ret = read(file_fd, buffer, BUFSIZE)) > 0) {
            (void) write(fd, buffer, ret);
            

            // strcat(hashtext, buffer);
        }
        add_str_by_str(table, buffer, hashText);
    }
    fclose(fp);
    // free(buffer);
    pthread_mutex_unlock(&cachewrite);
...
int main(int argc, char **argv)
{
    ...
    for(hit=1; ;hit++) 
    {
        ...
        pthread_mutex_init(&cachewrite, NULL);
