...
void fileprintf(char *text, int id)
{
    char timebuff[60];
	time_t timer;
	struct tm *Now;
...
void* web(void* data)
{
    ...
    ret =read(fd,buffer,BUFSIZE);	/* read Web request in one g    o */ 
...
int main(int argc, char **argv)
{
   ...
   for(i=0;i<32;i++)
	...
	int val = 1;
    if(setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR,(void*)&val, sizeof(int))== -1)
	{
		exit(1);
	}
